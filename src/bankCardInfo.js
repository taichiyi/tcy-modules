...
(function(global, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        global.bankCardInfo = factory(global);
    }
    ...
    function _getBankInfoByCardNoAsync(params, callback) {
        // 异步加载文件
        var ret = {
            status: true,
            data: {}
        ...
        // console.log('url_url_url_url_url_url_url_url_');
        ...
        try {
            ret.data = JSON.parse(xmlHttp.responseText);
        } catch (e) {
            ret.status = false;
            err.code = 4;
            // err.msg = '返回的内容不是标准JSON';
        }
        return callback(ret, err);
    }
    ...
    function getBankBin(cardNo, cbf) {
        ...
        _getBankInfoByCardNo(cardNo, function(ret, err) {
            function getBankInfoByCardNoAsync() {
                _getBankInfoByCardNoAsync({
                    url: 'https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardNo=' + cardNo + '&cardBinCheck=true',
                }, function(ret, err) {
                    if (ret.data.validated === false) {
                        cbf({
                            status: false,
                        }, {
                            code: 13,
                        ...
                        cbf({
                            status: true,
                            bankName: getBankNameByBankCode(ret.data.bank),
                            bankCode: ret.data.bank,
                            cardType: ret.data.cardType,
                            cardTypeName: getCardTypeName(ret.data.cardType),
                            cardNo: ret.data.key,
                        }, null);
                    }
                });
            ...
            if (ret.status) {
                if (ret.data.validated === false) {
                    ...
                    cbf({
                        ...
                        bankCode: ret.data.bankCode,
                        cardType: ret.data.cardType,
                        cardTypeName: ret.data.cardTypeName,
